...
<div class="float-end">
    ...
    <?php if ($this->request->getParam('action') === 'edit') : ?>
        <?= $this->Form->button(
            ...
            ['name' => 'redirect', 'value' => 'edit', 'class' => 'btn-outline-success', 'escapeTitle' => false],
        ) ?>
    <?php endif; ?>
    <?php if ($this->request->getParam('action') === 'add' && empty($this->request->getQueryParams())) : ?>
        <?= $this->Form->button(
            '<i class="fa-solid fa-plus-circle"></i> ' . __('Save & New'),
            ['name' => 'redirect', 'value' => 'add', 'class' => 'btn-outline-success', 'escapeTitle' => false],
        ) ?>
    <?php endif; ?>
</div>
